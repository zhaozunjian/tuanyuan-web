<template>
  <el-card body-style="padding:10px;height:100%;" shadow="never">
    <div slot="header" >
      <el-select class="sd-width-150" v-model="busine" @change="changeBus" placeholder="商品分成配置" size="small">
        <el-option v-for="item in busineArray" :key="item.code" :label="item.name" :value="item.code" />
      </el-select>
      <el-button type="primary" size="small" icon="el-icon-search" class="sd-mag-l-10" @click="getlist()" >查询</el-button>
    </div>
    <div v-if="busine == 11 || busine == 21">
      <el-table
        v-loading="vLoading"
  ...
  export default {
    data() {
      return {
        ...
        currPage:this.$GlobalApi.Constants.DICT.PAGE,
